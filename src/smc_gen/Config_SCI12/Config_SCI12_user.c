...
uint8_t buffer_rx[MAX_QUEUE_DATA_UART];
...
static void r_Config_SCI12_callback_receiveend(void)
{
	...
	g_rx_flag = 1U;
	/* Set SCI12 receive buffer address and restart reception */
	R_Config_SCI12_Serial_Receive((uint8_t *)&g_rx_char, 1);
	if(buffer_index < MAX_QUEUE_DATA_UART)
	{
		buffer_rx[buffer_index++] = g_rx_char;
	}
	else{
		buffer_index = 0;
	}
...
MD_STATUS R_SCI12_AsyncTransmit (uint8_t * const tx_buf, const uint16_t tx_num)
{
