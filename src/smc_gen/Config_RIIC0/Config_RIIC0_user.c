...
static volatile bool i2cReceive_done_flag = false;
...
static uint8_t ENV_crc_2wire_msb(const uint8_t buf[], int len) {
    uint8_t crc = 0xFF;
    for (uint8_t i = 0; i < len; i++) {
        crc ^= buf[i];
        for (uint8_t j = 0; j < 8; j++) {
            if ((crc & 0x80u) != 0) {
                crc = (uint8_t)((uint8_t)(crc << 1u) ^ 0x31);
            } else {
                crc <<= 1u;
            }
        }
...
void readSensorSHT30()
{
	uint8_t arr[6] = {0};
	AIRC_STATUS_t *p_s = APL_stt_get();
	uint16_t humid_code = 0, temp_code = 0;
	int32_t tmp;
	uint8_t cal_checksum = 0;
	arr[0] = 0x24;
	arr[1] = 0x16;
	R_Config_RIIC0_Master_Send(0x44 , arr, 2);
	R_BSP_SoftwareDelay(16,BSP_DELAY_MILLISECS);

	i2cReceive_done_flag = false;
	R_Config_RIIC0_Master_Receive(0x44, arr, 6);
	while (i2cReceive_done_flag == false){};

	cal_checksum = ENV_crc_2wire_msb(arr, 2);
	if (cal_checksum != arr[2]) {
		p_s->temp_indoor = 0;
	}
	else
	{
		temp_code = (uint16_t) arr[0] << 8 | arr[1];
		tmp = (((uint32_t)temp_code * 175) / 0xFFFF) - 45;
		p_s->temp_indoor = tmp;
	}
	cal_checksum = ENV_crc_2wire_msb(&arr[3], 2);
	if (cal_checksum != arr[5]) {
		p_s->humid_indoor = 0;
	}
	else
	{
		humid_code = (uint16_t) arr[3] << 8 | arr[4];
		tmp = ((uint32_t)humid_code * 100) / 0xFFFF;
		p_s->humid_indoor = tmp;
	}
	if ((p_s->temp_indoor == 0) || (p_s->temp_indoor > 99))
	{
		error_sensor |= (1<<8);
	}
	else
	...
	if ((p_s->humid_indoor == 0) || (p_s->humid_indoor > 99))
	{
		error_sensor |= (1<<9);
	}
	else
