...
#define  FLASH_SEL_INACTIVE    	R_GPIO_PinWrite(GPIO_PORT_C_PIN_4, GPIO_LEVEL_HIGH);
#define  FLASH_SEL_ACTIVE 		R_GPIO_PinWrite(GPIO_PORT_C_PIN_4, GPIO_LEVEL_LOW);
...
void deviceFlash_eraseSector(uint32_t add)
{
	uint16_t arr[4];
	uint16_t *ret = malloc((sizeof(uint16_t))*4);
	arr[0] = sector4k;
